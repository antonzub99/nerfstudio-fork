...
from nerfstudio.process_data.colmap_utils import read_points3d_binary, read_images_binary
...
@dataclass
class NerfstudioDSDataParserConfig(DataParserConfig):
    """Nerfstudio dataset config"""

    _target: Type = field(default_factory=lambda: NerfstudioDS)
    """target class to instantiate"""
    data: Path = Path("data/nerfstudio/poster")
    """Directory or explicit json file path specifying location of data."""
    use_depth: bool = True
    """If True, depth supervision from COLAMP points is used"""
...
@dataclass
class NerfstudioDS(DataParser):
    """Nerfstudio depth-supervised DatasetParser"""

    config: NerfstudioDSDataParserConfig
    downscale_factor: Optional[int] = None
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        if self.config.use_depth:
            ...
            sparse_data = self.config.data / "colmap" / "sparse" / "0"
            ...
            for im_id in range(1, len(sparse_images)+1):
                depths, coords, weights = [] , [], []
                for p_idx in range(len(sparse_images[im_id].xys)):
                    point2d = sparse_images[im_id].xys[p_idx]
                    id_3d = sparse_images[im_id].point3d_ids[p_idx]
                    if id_3d == -1:
                        continue
            ...
            metadata["depth_data"] = []
            for idx in indices:
                metadata["depth_data"].append(depth_data[idx])
             #filter by train\test split
            
            # number of keypoints is different in different images, so i crop them to the lowest number ...
            
            check_depths = []
            for item in metadata["depth_data"]:
                check_depths.append(len(item["depth"]))
            min_kpts = np.amin(check_depths)
            
            new_data = []
            for item in metadata["depth_data"]:
                new_item = dict()
                for key, val in item.items():
                    new_item[key] = val[:min_kpts]
                new_data.append(new_item)
            metadata["depth_data"] = new_data
            
            #max_kpts = np.amax(check_depths)
