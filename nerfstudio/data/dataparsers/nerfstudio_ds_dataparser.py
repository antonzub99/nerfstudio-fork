...
class NerfstudioDS(DataParser):
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        if self.use_depth:
            ...
            sparse_data = self.data / "colmap" / "sparse" / "0"
            ...
            for im_id in range(1, len(sparse_images)+1):
                ...
                for p_idx in range(len(sparse_images[im_id].xys)):
                    ...
                    if id_3d == -1:
                        continue
                    point3d = sparse_points[id_3d].xyz
                    ...
                    weight = 2 * np.exp(-(errors / mean_error)**2)
                    depths.append(depth)
                    coords.append(point2d / self.downscale_factor)
                    weights.append(weight)
                depth_data.append({"depth": np.array(depths), "coord": np.array(coords), "weight": np.array("weight")})
            
            metadata["depth_data"] = depth_data[indices] #filter by train\test split
